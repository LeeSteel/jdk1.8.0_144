...
public class HashMap<K,V> extends AbstractMap<K,V>
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;//如果内置的table 为空，或者tab长度为0，重新初始化
        if ((p = tab[i = (n - 1) & hash]) == null) //如果 当前数组下标没有元素，添加一个新的 Node 节点
            tab[i] = newNode(hash, key, value, null);// 添加一个新的 Node 节点
        else { // 当前下标 已有Node 元素，
            Node<K,V> e; K k;
            if (p.hash == hash &&
                ((k = p.key) == key || (key != null && key.equals(k)))) // 判断是否 hash 相同，如果hash相同，判断key 是否相同
                e = p; // 如果key 相同，取出旧数据
            else if (p instanceof TreeNode) //判断 当前元素 是否 树节点
            ...
            if (e != null) { // existing mapping for key  已经存在对应的key
                V oldValue = e.value;
     ...
     * table is too small, in which case resizes instead. 除非给定表太小，否则将替换给定哈希值的索引中bin中所有链接的节点，在这种情况下，将调整大小。
     */
    final void treeifyBin(Node<K,V>[] tab, int hash) {
        int n, index; Node<K,V> e;
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY) //如果tab 为空, 或者小于最小树容量，扩容
            resize();
        else if ((e = tab[index = (n - 1) & hash]) != null) {
            TreeNode<K,V> hd = null, tl = null;
            do {
                TreeNode<K,V> p = replacementTreeNode(e, null); //替换当前节点为 树节点
                if (tl == null)
                    hd = p;
                else {
                    p.prev = tl;
